...
# Verify with: adb shell uiautomator dump /dev/stdout | grep -i "momo\|input\|send\|unread"
_PKG = "com.immomo.momo"
_CHAT_LIST_ID = f"{_PKG}:id/listView"
_UNREAD_BADGE_ID = f"{_PKG}:id/badge"
_CONTACT_NAME_ID = f"{_PKG}:id/tv_name"
_INPUT_ID = f"{_PKG}:id/input_view"
_SEND_BTN_ID = f"{_PKG}:id/send_btn"


class MomoConnector(AndroidBaseConnector):
    app_name = "momo"
    package_name = _PKG
    channel = "momo"
    ...
    def _read_conversation(self, device, contact_id: str) -> list[AndroidMessage]:
        ...
        try:
            ...
            index = 0
            items = device(className="android.widget.ListView").child(className="android.widget.FrameLayout")
            for item in items:
